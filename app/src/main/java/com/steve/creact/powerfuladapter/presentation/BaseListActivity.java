...
public class BaseListActivity extends AppCompatActivity {
  ...
  /**
   * Extend CommonRecyclerAdapter if you want set holder'listener outside of the ViewHolder
   ...
   */
  private class CustomAdapter extends CommonRecyclerAdapter {

    @Override
    public void setListener( BaseRecyclerViewHolder holder, DisplayBean bindBean, int position ) {
      if ( bindBean instanceof BookTitleBean ) {
        final Book book = ( ( BookTitleBean ) bindBean ).getData();
        holder.setOnItemClickListener( new View.OnClickListener() {
          @Override
          public void onClick( View v ) {
            Toast.makeText( BaseListActivity.this, "clicked " + book == null ? "" : book.getName(), Toast.LENGTH_SHORT ).show();
          }
        } );
      }
    }
